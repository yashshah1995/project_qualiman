#' R script for all the helper functions needed throughout the
#' project. Add a new common required function here.

box::use(stringr[str_c],dplyr[filter,select,distinct,arrange,slice])
...
make_next <- function(str){
  if(substrRight(str,2)=="WS"){
    str2 <- str_c((as.integer(substr(str, 1, 2))+1),"SS")
  }else{
    str2 <- str_c(as.integer(substr(str, 1, 2)),"WS")
  }
  return(str2)
}
...
get_columns <- function(col){
  ...
  for(str in unique(col)){
    str <- str_c(substrRight(str,2),substr(str, 1, 2))
    sem <- c(sem,str)
  }
  min <- min(unique(sem))
  max <- max(unique(sem))
  ...
  str <- min
  while(str != max){
    sem <- c(sem,str_c(substrRight(str,2),substr(str, 1, 2)))
    str <- make_next(str)
  }
  sem <- c(sem,str_c(substrRight(max,2),substr(max, 1, 2)))
  return(sem)
}
...
get_sortable_sem <-function(str){
  ...
  # #print(str)
  #
  # #print(str_c(substrRight(str,2),ifelse(toupper(substr(str, 1, 2))=="SS",".1",".2")))
  #
  #
  # return(str_c(substrRight(str,2),ifelse(toupper(substr(str, 1, 2))=="SS",".1",".2")))
...
get_unsortable_sem <-function(str){
  ...
  return(str_c(ifelse( substrRight(str,2)==".1","SS","WS"),substr(str, 1, 2)))
}
...
get_unsortable_old_sem <-function(str){
  ...
  return(str_c(ifelse( substrRight(str,2)==".1","SS","WS")," ",substr(str, 1, 2),ifelse( substrRight(str,2)==".1","", as.integer(substr(str, 1, 2))+1)))
}
...
#   tmp_str_x     <- str_c("ECTS (",semester,")")
#   tmp_str_y     <- "Number of Students"
#   ###
#   gg <- ggplot(tmp_data %>%
#                  filter(Semester==semester) ,aes(x=CP)) +
...
#   gwidth   <- 40
#   gheight  <- 15
#   gdate    <- str_c("_",semester)          # "_(TT.MM.2018)" e.g. _(14.05.2018)
#   ###
#   gname    <- str_c(gtype,"_",gname,"_",project_short) # e.g. Hist_ECTS_QM3_PNG_(14.05.2018).png
#   if(flag_save_plots)     {ggsave(str_c(pdf_folder,gname,       gdate,".pdf"), width = gwidth, height = gheight, units = "cm") }
#   if(flag_save_plots_svg) {ggsave(str_c(svg_folder,gname,"_SVG",gdate,".svg"), width = gwidth, height = gheight, units = "cm") }
#   if(flag_save_plots_png) {ggsave(str_c(png_folder,gname,"_PNG",gdate,".png"), width = gwidth, height = gheight, units = "cm") }
...
#' @export
make_plot_label <- function(semester_col){
  result <- vector(mode="character", length=0)
  for (var in semester_col){
    if(is.na(var) | var==""){
      num <- "NA"
    }else{
      if (var == "WiSe 2014" || var == "WS14" || var =="WiSe 14" || var =="WS 1415"   || var =="2014 WiSe") { num <- "WiSe 14/15"}
      if (var == "SoSe 2015" || var == "SS15" || var =="SoSe 15" || var =="SS 15"   || var =="2015 SoSe") { num <- "SoSe 15"}
      if (var == "WiSe 2015" || var == "WS15" || var =="WiSe 15" || var =="WS 1516" || var =="2015/2016 WiSe") { num <- "WiSe 15/16"}
      if (var == "SoSe 2016" || var == "SS16" || var =="SoSe 16" || var =="SS 16"   || var =="2016 SoSe") { num <- "SoSe 16"}
      if (var == "WiSe 2016" || var == "WS16" || var =="WiSe 16" || var =="WS 1617" || var =="2016/2017 WiSe") { num <- "WiSe 16/17"}
      if (var == "SoSe 2017" || var == "SS17" || var =="SoSe 17" || var =="SS 17"   || var =="2017 SoSe") { num <- "SoSe 17"}
      if (var == "WiSe 2017" || var == "WS17" || var =="WiSe 17" || var =="WS 1718" || var =="2017/2018 WiSe") { num <- "WiSe 17/18"}
      if (var == "SoSe 2018" || var == "SS18" || var =="SoSe 18" || var =="SS 18"   || var =="2018 SoSe") { num <- "SoSe 18"}
      if (var == "WiSe 2018" || var == "WS18" || var =="WiSe 18" || var =="WS 1819" || var =="2018/2019 WiSe") { num <- "WiSe 18/19"}
      if (var == "SoSe 2019" || var == "SS19" || var =="SoSe 19" || var =="SS 19"   || var =="2019 SoSe") { num <- "SoSe 19"}
      if (var == "WiSe 2019" || var == "WS19" || var =="WiSe 19" || var =="WS 1920" || var =="2019 WiSe") { num <- "WiSe 19/20"}
      if (var == "SoSe 2020" || var == "SS20" || var =="SoSe 20" || var =="SS 20" || var =="2020 SoSe") { num <- "SoSe 20"}
      if (var == "WiSe 2020" || var == "WS20" || var =="WiSe 20" || var =="WS 2021" || var =="2020 WiSe") { num <- "WiSe 20/21"}
    }
    result <- c(result, num)
  }#for
  return(result)
...
# This function is one of the most important, Whenever a new semester data occurs,
# We need to add a new num in different formats a data can occur.
...
make_semester_to_num <- function(semester_col){
  ...
  for (var in semester_col){
    if(is.na(var) | var==""){
      ...
      if (var == "SoSe 2020" || var == "SS20" || var =="SoSe 20" || var =="SS 20" || var =="2020 SoSe") { num <- 11}
      if (var == "WiSe 2020" || var == "WS20" || var =="WiSe 20" || var =="WS 2021" || var =="2020 WiSe") { num <- 12}
    }
    result <- c(result, num)
  }#for
  return(result)
...
create_semester_to_num <- function(){
  ...
  #   check <- TRUE
  #   lu <- read.csv(str_c(data_folder_sub,"lu",".csv"),stringsAsFactors=FALSE)
  # }

  last_semester_project <-  lu %>%
    filter(LV.Datum.Semester != "NA") %>%
    select("LV.Datum.Semester") %>%
    distinct()

  last_semester_project <- as.data.frame(last_semester_project)

  last_semester_project <- last_semester_project %>%
    filter( !(last_semester_project$LV.Datum.Semester %in% vec_drop_sem_before_start(last_semester_project$LV.Datum.Semester) ) )
  ...
  while(cur != last_semester_project){
    ...
    if(sem == 1){
      cur <- as.character(str_c(year,".","2"))
    }
    if(sem == 2){
      cur <- as.character(str_c( as.integer(year+1),".","1"))
    }
    sem_num <- sem_num + 1
...
get_semester_number_from_string <- function(str){
  ...
  # if(!exists("df_semester_translation")) {
  #   check <- TRUE
  #   df_semester_translation <- read.csv(str_c(data_folder_sub,"df_semester_translation",".csv"),stringsAsFactors=FALSE)
  # }
